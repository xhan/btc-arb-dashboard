...
const fixedUsdgRule = FIXED_PATH_RULES.find((entry) => entry.id === 'fixed:usdg-usdc');
...
assert.ok(fixedUsdgRule, 'missing fixed rule: fixed:usdg-usdc');
assert.strictEqual(fixedUsdgRule.title, 'USDG - USDC');
assert.deepStrictEqual(fixedUsdgRule.categoryNames, ['USD监控']);
assert.strictEqual(fixedUsdgRule.base, 'USDG');
assert.strictEqual(fixedUsdgRule.quote, 'USDC');
assert.strictEqual(fixedUsdgRule.resultLimit, 2);
...
assert.ok(usdgUsdcAlert, 'missing alert for fixed:usdg-usdc');
assert.strictEqual(usdgUsdcAlert.enabled, true);
assert.strictEqual(usdgUsdcAlert.thresholdBp, 1);
