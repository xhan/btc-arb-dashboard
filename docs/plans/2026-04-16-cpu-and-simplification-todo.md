...
- 建议拆分：
  ...
  - `dashboard-modal-dom`：add quote 弹窗的 DOM 读值、show/hide 写入、placeholder/disabled 写入和 reset 适配层已下沉到 `src/dashboard/dashboard-modal-utils.js`，主入口只保留新增 quote 后的 dashboard 状态更新和刷新调度
