...
- 建议拆分：
  ...
  - `quote-ui-runtime`：hover 延迟显示、tooltip 展示模型、request channel tag DOM patch、trend arrow 展示模型和 trend timer 已分别下沉到 `src/quote/quote-display-utils.js` / `src/quote/quote-state-runtime-utils.js`，tooltip 定位/显示、trend arrow DOM patch/reset 和 quote alert highlight 应用/清理已下沉到 `src/ui/dom-render-utils.js`
...
- 现状：
  ...
  - muted target / leg 列表和 timer 所有权已下沉到 `src/path-alerts/muted-path-runtime-utils.js`，`src/app/dashboard-app.js` 保留持久化、UI 渲染和联动刷新回调
- 建议：
  - 后续如果继续拆 `src/app/dashboard-app.js`，把持久化和 UI 回调继续收拢到 `path-alerts` 模块
