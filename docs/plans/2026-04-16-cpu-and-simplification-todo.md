...
- 建议拆分：
  ...
  - `arb-alert-bridge`：报警触发后定位/高亮套利机会、muted leg 变化后的套利缓存失效、面板刷新和详情关闭/重绘，以及 arb panel 读取 muted legs / muted target key 的桥接已下沉到 `src/arb/arb-alert-bridge-utils.js`；`alert-runtime-controller` 不再直接持有 arb opportunity/highlight runtime 或直接调用 arb panel/detail 刷新函数，`arb-panel-controller` 也不再直接读取 alert runtime
  ...
  - `app-lifecycle`：启动加载、配置预加载、主看板根节点事件、一次性全局事件绑定、浮窗 chrome 绑定和快捷键绑定已下沉到 `src/app/dashboard-lifecycle-controller.js`；全局快捷键已提前到静态事件阶段绑定且 controller 自身幂等，为后续统一 command/view lifecycle、解决首次点击/快捷键不稳定提供单一入口
...
- 现状：
  ...
  - alert log 的 muted target key 点击解析已下沉到 `src/alerts/alert-log-ui-utils.js`，主入口只消费 action 并执行延长/屏蔽业务
  ...
  - `src/app/dashboard-app.js` 的 request channel tag 默认 DOM adapter 和 dashboard 批量刷新已下沉到 `src/request-channel/request-channel-utils.js` 的 `createRequestChannelRuntime()`
  ...
  - `src/app/dashboard-app.js` 的套利全局过滤栏状态/DOM 同步单层包装已移除，调用点直接使用 filter runtime 和 layout utils
  - `src/app/dashboard-app.js` 的套利全局过滤栏 patch/clear 事件包装已移除，事件绑定处直接委托 filter runtime 并触发面板刷新
  ...
  - `src/app/dashboard-app.js` 的 path alert display title 单用途包装已移除，标题 fallback 直接内联到 triggered entry 构造配置
  - `src/app/dashboard-app.js` 的 muted target candidate 单用途包装已移除，triggered entry 构造配置直接委托 notification utils
  - `src/app/dashboard-app.js` 的 path alert active evaluation target/list 单用途包装已移除，调度器和评估入口直接委托 dashboard runtime utils
  - `src/app/dashboard-app.js` 的 path alert real leg count 单用途包装已移除，triggered entry 和排序配置直接委托 path alert utils
  ...
  - `src/app/dashboard-app.js` 的 quote UI state 写入单用途包装已移除，高亮确认和 dismiss 流程直接委托 quote state runtime
  ...
  - `src/app/dashboard-app.js` 的报价设置 request-channel options 渲染单用途包装已移除，打开设置弹窗时直接委托 request-channel / modal utils
