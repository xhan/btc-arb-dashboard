...
- 建议拆分：
  ...
  - `app-lifecycle`：启动加载、配置预加载、主看板根节点事件、一次性全局事件绑定、浮窗 chrome 绑定和快捷键绑定已下沉到 `src/app/dashboard-lifecycle-controller.js`，为后续统一 command/view lifecycle、解决首次点击/快捷键不稳定提供单一入口
