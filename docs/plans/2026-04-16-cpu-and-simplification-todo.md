...
- 建议拆分：
  ...
  - `arb-alert-bridge`：报警触发后定位/高亮套利机会、muted leg 变化后的套利缓存失效、面板刷新和详情关闭/重绘桥接已下沉到 `src/arb/arb-alert-bridge-utils.js`，`alert-runtime-controller` 不再直接持有 arb opportunity/highlight runtime，也不直接调用 arb panel/detail 刷新函数
