...
- 建议拆分：
  ...
  - `quote-ui-runtime`：hover 延迟显示、tooltip 展示模型、request channel tag DOM patch、trend arrow 展示模型和 trend timer 已分别下沉到 `src/quote/quote-display-utils.js` / `src/quote/quote-state-runtime-utils.js`，tooltip 定位/显示、trend arrow DOM patch/reset 和 quote highlight 清理已下沉到 `src/ui/dom-render-utils.js`
