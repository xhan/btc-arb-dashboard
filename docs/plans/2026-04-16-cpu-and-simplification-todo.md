...
- 建议拆分：
  ...
  - `dashboard-persistence`：配置保存 debounce timer、金额输入 debounce、保存按钮反馈 runtime 已下沉到 `dashboard-runtime-utils.js`，dashboard 输入/按钮动作解析、添加报价表单状态和全局设置 interval 表单读写/解析已下沉到 `dashboard-renderer.js`
