...
- 建议拆分：
  ...
  - `floating-panel-ui`：浮窗拖拽、显示切换、置顶绑定和 z-index runtime 已下沉到 `src/ui/dom-render-utils.js`，全局快捷键解析和输入目标判断已下沉到 `src/ui/keyboard-shortcut-utils.js`，`src/app/dashboard-app.js` 只保留快捷键动作分发和面板打开后的业务刷新
...
- 现状：
  ...
  - `src/app/dashboard-app.js` 的数据终端 control/selection patch 状态应用已下沉到 `src/data-terminal/data-terminal-utils.js`
  - `src/app/dashboard-app.js` 的数据终端 DOM refs 装配、浮窗定位和默认尺寸 DOM 写入已下沉到 `src/data-terminal/data-terminal-utils.js`
  ...
  - `src/app/dashboard-app.js` 的套利详情 chart/profit preview 清空、消息、strip、收益卡片 HTML、canvas/meta 查询、meta 文案和图表错误 DOM 写入已下沉到 `src/arb/arb-detail-utils.js`
